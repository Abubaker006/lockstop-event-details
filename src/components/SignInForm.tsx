...
import { CustomErrorResponse, requestOtp, verifyOtp, APIRESPONSELOGIN } from "@/apiService/apiServices";
...
const SignInForm = () => {
    ...
    const handleSubmitOTP = async (values: OtpValues, actions: FormikHelpers<OtpValues>) => {
        if (values.otpNumber.length === 4) {
            try {
                const response: APIRESPONSELOGIN = await verifyOtp(fullPhone, values.otpNumber);
    ...
    const handleResendOtp = async () => {
        ...
        try {
            ...
            if (response.success) {
                ...
                setErrorMessage(response.error || "Failed to resend OTP.");
            }
        } catch (error) {
            const axiosError = error as AxiosError<{ message?: string }>;
            setErrorMessage(
    ...
    return (
        <>
                                ...
                                <div className="flex space-x-2 items-stretch pt-2 pb-2">
                                    <div className=''>
                                        ...
                                        >
                                            ...
                                            {countries.map((code, idx) => (<option key={idx} value={code.value}>{code.label}</option>))}
                                        </Field>
            ...
            {isOTPVisible && (
                <Modal className='m-auto' open={isOTPVisible} footer={false} onCancel={() => { setisOTPVisible(!isOTPVisible); setErrorMessage(""); }} onClose={() => { setisOTPVisible(!isOTPVisible); setErrorMessage(""); }}>
                                ...
                                >
                                    ...
                                    <div className="flex justify-center gap-6 mb-5">
                                        {otp.map((digit, idx) => (
                                            <input
                                                ...
                                                maxLength={1}
                                                value={digit}
                                                onChange={(e) => handleChange(e.target.value, idx, setFieldValue)}
                                    ...
                                    <div className="mt-4 text-center text-sm text-gray-500">
                                        ...
                                        <button
                                            ...
                                            className={`text-blue-600 hover:underline ${resendCooldown > 0 ? "opacity-50 cursor-not-allowed" : "cursor-pointer"
                                                }`}
                                        >
